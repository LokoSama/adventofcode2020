const stringList = "eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241\n" +
    "\n" +
...
function validate(passportData) {
    return requiredFields.every(field => passportData.filter(data => data.split(":")[0] === field).length > 0)
}
list = list.map(elt =>
{
    return elt.split('\n').join(' ')
...
const result = list.map(elt => elt.split(' '))
let valid = 0
result.forEach(elt => {
    if (validate(elt)) {valid ++}})
console.log("PART1 RESULT: " + valid)

// PART 2
function validateByr(elt) {
    return elt[0] === "byr" && elt[1].length === 4 && parseInt(elt[1]) >= 1920 && parseInt(elt[1]) <= 2002
}

function validateIyr(elt) {
    return elt[0] === "iyr" && elt[1].length === 4 && parseInt(elt[1]) >= 2010 && parseInt(elt[1]) <= 2020
}

function validateEyr(elt) {
    return elt[0] === "eyr" && elt[1].length === 4 && parseInt(elt[1]) >= 2020 && parseInt(elt[1]) <= 2030
}

function validateHgt(elt) {
    if (!elt[1] || elt[1].length < 3 || elt[0] !== "hgt"){
        return false
    }
    if (elt[1].slice(-2) === "cm"){
        return parseInt(elt[1].slice(0,-2)) >= 150 && parseInt(elt[1].slice(0,-2)) <= 193
    } else if (elt[1].slice(-2) === "in"){
        return parseInt(elt[1].slice(0,-2)) >= 59 && parseInt(elt[1].slice(0,-2)) <= 76
    } else {
        return false
    }
}

function validateHcl(elt) {
    const result = elt[0] === 'hcl' && elt[1].match(/^#[a-f0-9]{6}$/i) !== null;
    return result
}

function validateEcl(elt) {
    if (elt[0] !== 'ecl') return false
    const validColors = ['amb','blu','brn','gry','grn','hzl','oth']
    return !!validColors.some((color) => elt[1] === color);
}

function validatePid(elt) {
    return elt[0] === 'pid' && elt[1].match(/^[0-9]{9}$/i) !== null;
}


function validate2(passportData) { // long statements to make debug easier
    if (!passportData.filter((elt) => validateByr(elt.split(":"))).length > 0){
        return false
    }
    if (!passportData.filter((elt) => validateEyr(elt.split(":"))).length > 0){
        return false
    }
    if (!passportData.filter((elt) => validateIyr(elt.split(":"))).length > 0){
        return false
    }
    if (!passportData.filter((elt) => validateHgt(elt.split(":"))).length > 0){
        return false
    }
    if (!passportData.filter((elt) => validateHcl(elt.split(":"))).length > 0){
        return false
    }
    if (!passportData.filter((elt) => validateEcl(elt.split(":"))).length > 0){
        return false
    }
    return !passportData.filter((elt) => validatePid(elt.split(":"))).length <= 0
...
result.forEach(elt => {
    if (validate2(elt)) {
        ...
        console.log("FAIL" + elt)
    }})
console.log("PART2 RESULT: " + valid)
